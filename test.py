...
expected_great: list = [{'rank': 5, 'percentage': 99.839, 'pokemon': '2', 'form': '166', 'level': '39', 'cp': 1500},
                        {'rank': 1, 'percentage': 100.0, 'pokemon': '3', 'form': '169', 'level': '21', 'cp': 1498}]
expected_ultra: list = [{'rank': 33, 'percentage': 99.189, 'pokemon': '3', 'form': '169', 'level': '40', 'cp': 2498}, ]
expected_result: tuple = (expected_great, expected_ultra)
custom_values = False
if len(sys.argv) > 1:
    try:
        ...
        logger.warning("Parsing custom values failed. Using a default Bulbasaur test: 0/14/11 @ Lvl 2 - this should "
                       f"return the following result: {expected_result}")
else:
...
if not custom_values:
    logger.info("To run a custom test, pass the following values as arguments, separated by spaces: mon_id, form_id, "
                "attack_iv, defense_iv, stamina_iv, level. Example of the default test: "
